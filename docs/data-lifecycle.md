...
| 2. Collect        | `radar/collect/` — Kurir RSS, Kurir's sitemap archive, CRTA's published reports (162 rows / 76 headlines).                 | Kurir only for live headlines. Check robots.txt and terms for `crta.plus` and each new outlet before scraping. RSS is the polite default. Record what was checked.  |
| 3. Preprocess     | `radar/prepare/` — Cyrillic/Latin + diacritic normalisation, outlet canonicalisation, raw→store ingest. One canonical store, deduped on (url, headline, outlet). | Near-duplicate clustering only exists for CRTA rows (`cluster_id`); RSS-collected headlines have none, so cluster-grouped splits don't cover them yet.              |
