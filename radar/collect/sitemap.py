...
Checks headlines against the canonical store so nothing gets sent through
twice. Overwrites data/interim/llm_batch_pending.csv each run - it's a
staging file, not a record, so there's nothing to lose by clobbering it.
"""
...
OUT = LLM_BATCH_PENDING


def already_seen():
    """Headlines already in the canonical store, whoever labelled them."""
